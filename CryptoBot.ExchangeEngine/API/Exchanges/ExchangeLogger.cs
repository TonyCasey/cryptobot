...
namespace CryptoBot.ExchangeEngine.API.Exchanges
{
    ...
    public class ExchangeLogger : IDisposable
    {
        ...
        public void Update()
        {
            ...
            try
            {
                if (Symbol == "*")
                {
                    ...
                    foreach (KeyValuePair<string, Ticker> ticker in Tickers)
                    {
                ...
                else
                {
                    ...
                    Tickers = new KeyValuePair<string, Ticker>[1] { new KeyValuePair<string, Ticker>(Symbol, API.GetTicker(BaseCoin, Coin)) };
                    OrderBook = API.GetOrderBook(BaseCoin, Coin);
                    Trades = API.GetRecentTrades(BaseCoin, Coin).OrderBy(t => t.Timestamp).ToArray();
        ...
        public static IEnumerable<Dictionary<string, Ticker>> ReadMultiTickers(string path)
        {
            int count;
        ...
        /// Latest tickers
        /// </summary>
        public IReadOnlyCollection<KeyValuePair<string, Ticker>> Tickers { get; private set; }
